...
// Hàm đọc nội dung của tệp tin dưới dạng văn bản và trả về một Promise
...
async function enCodeDes() {
  const key = document.getElementById("encode-key-input").value;
  const message = document.getElementById("encode-plant-text").value;
  ...
  if (file) {
    ...
    cipherText = des(key, fileContent, 1, 0);
    console.log(cipherText);

    plantText = des(key, cipherText, 0);
    // Hiển thị kết quả
    document.getElementById("encode-cipher-text").value =
      stringToHex(cipherText);

    // document.getElementById("decode-plant-text").value = plantText;
    //     return;
  ...
  cipherText = des(key, message, 1, 0);
  plantText = des(key, cipherText, 0);

  document.getElementById("encode-cipher-text").value = stringToHex(cipherText);
}

async function decodeDes() {
  const key = document.getElementById("decode-key-input").value;
  const message = document.getElementById("decode-plant-text").value;
  const fileInput = document.getElementById("decode-file-input");
  const file = fileInput.files[0];

  console.log("key", key);
  console.log("message", message);
  var decodeText = des(key, message, 0);
  ...
  if (file) {
    ...
    plantText = des(key, fileContent, 0);
  ...
  plantText = des(key, hexToString(message), 0);
  console.log(plantText);
  document.getElementById("decode-cipher-text").value = plantText;
}

// var key = "this is a 24 byte key !!";
// var message = "ten toi la le cong nghia aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
// var ciphertext = des(key, message, 1, 0);
// console.log(ciphertext);
// // var decodeText = des(key, ciphertext, false, 1, 0);
// console.log(ciphertext);
// var decodeText = des(key, ciphertext, 0);
// console.log("DES Test: " + stringToHex(ciphertext));
// console.log("DES Test: " + decodeText);
